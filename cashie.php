...
$slug = ($_GET['category'] ? $_GET['category'] : 0);
$slug = ($_GET['CATEGORY'] ? $_GET['CATEGORY'] : $slug);
...
if ($cid) {
	...
	$desc  = $product['seo_meta_description'];
	$keys  = $product['seo_meta_keywords'];
	$head  = $product['seo_page_heading'];
}
if ($slug) {
	...
	$array = $_cashieData['categories']; if ($array) { reset($array); while (list($index,$ob)=each($array)) {
		$category[$ob['slug']] = $ob;
	}}
	$seo = $category[$slug];
	
	$title = $seo['seo_page_title'];
	$desc  = $seo['seo_meta_description'];
	$keys  = $seo['seo_meta_keywords'];
	$head  = $seo['seo_page_heading'];
}
...
if (!class_exists('cashie'))
{
	class cashie {
		...
		function init() {
			global $title, $desc, $keys, $head;
			if ($title) add_action('wp_title', array('cashie', 'set_title'));
			if ($desc)  add_action('wp_head',  array('cashie', 'set_keywords'));
			if ($keys)  add_action('wp_head',  array('cashie', 'set_description'));
			if ($head)  add_action('wp_head',  array('cashie', 'hide_h1'));
		}
...
add_action('plugins_loaded', array('cashie', 'init'));
