	...
	class cashie_settings
	{
		...
		function init_options()
		{
			...
			else
			{
				...
				$this->option_values = get_option($cashie_options_handle);
			}
		...
		function get_product_details ($args) {
			...
			if ($cid > 0 && $hash !== 0  && $catalog_id !== $page_id) {
				...
				echo '<h1>'.$product['seo_page_heading'].'</h1><br>';
			...
			if ($catalog_id == $page_id) {
				...
				foreach ($_cashieData['categories'] as $key => $category) {
				
					if ($category['slug']==$slug) {
						echo '<h1>'.$category['seo_page_title'].'</h1><br>';
					}
